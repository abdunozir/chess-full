...
board_cards.forEach((el) => {
  el.addEventListener('click', (e) => {
    if (
      e.currentTarget.classList[e.currentTarget.classList.length - 1].split(
        '-'
      )[0] == 'qora'
    ) {
      removeColor();
...
const sbc = (ev) => {
  ...
  true_place.forEach((item) => {
    item.removeEventListener('dblclick', (ev) => sbc(ev, e, clickedCardClass));
  });
};
...
function moveAblePlace() {
  true_place.forEach((item) => {
    item.addEventListener('dblclick', eventLis);
  });
}
